...
\begin{abstract}
A dataset to train a supervised learning algorithm comprises features and their corresponding labels. The labeling process is usually carried out by an expert, which provides the ground truth/gold standard for each sample. However, in many real-world applications, instead of such a gold standard, we usually have access to annotations provided by crowds, which hold different and unknown expertise levels. Thus, \emph{learning from crowds} is an emerging area in the machine learning scenario, which has the main aim to face different machine learning paradigms in the presence of multi-labelers information. Most state-of-the-art approaches reside on two key assumptions: i) the labeler’s performance does not depend on the input feature space, and ii) independence among the annotators is imposed. To deal with multiple annotators' problems, we propose the chained Gaussian processes from multiple annotators--(CGPMA) that allow modeling each annotator's performance as a function of the input space. Likewise, to exploit correlations in multiple labelers' outputs, we propose the correlated chained Gaussian processes from multiple annotators--(CGPMA) as an extension of CGPMA. Experimental results associated with regression and classification tasks over synthetic and real annotators show that our CGPMA and CCGPMA achieve suitable performances from inconsistent labelers, even if the gold standard is not available.
\end{abstract}
