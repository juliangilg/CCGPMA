...
def latent_functions_prior(Q, lenghtscale=None, variance=None, input_dim=None):
    ...
    kern_list = []
    for q in range(Q):
        kern_q = kern.RBF(input_dim=input_dim, lengthscale=lenghtscale[q], variance=variance[q], name='rbf')+ kern.White(input_dim, variance=1e-8)# \
...
def Chained_W_kappas(Q,D,rank, experiment=False):
    W_list = []
    kappa_list = []
    for q in range(Q):
        Ws = np.zeros((Q,1))
