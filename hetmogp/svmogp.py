...
class SVMOGP(GPy.core.SparseGP):
    def __init__(self, X, Y, Z, kern_list, likelihood, Y_metadata, name='SVMOGP', batch_size=None, non_chained=True):
        ...
        if non_chained:
               ...
               self.W_list, self.kappa_list = util.Chained_W_kappas(self.num_latent_funcs, self.num_output_funcs, rank=1)
        ...
        # Batch the data
        self.Xmulti_all, self.Ymulti_all, self.iAnn_all = X, Y, Y_metadata['iAnn']
    ...
    def set_data(self, X, Y, iAnn):
        ...
        """
        self.Xmulti, self.Ymulti, self.iAnnMulti = X, Y, iAnn
        self.Y_metadata.update(iAnn = iAnn)
        return 
    ...
    def new_batch(self):
        ...
        for t in range(T):
            ...
            iAnnmulti_batch.append(self.iAnn_all[t][i_task])
        return Xmulti_batch, Ymulti_batch, iAnnmulti_batch
    ...
    def predictive_new(self, Xnew, output_function_ind):
        """
        ...
        for t in range(len(self.likelihood.likelihoods_list)):
            if t == output_function_ind:
                _,num_f_task,_ = self.likelihood.likelihoods_list[t].get_metadata(self.iAnn_all[t])
        ...
        Y_metad = self.Y_metadata
        m_pred, v_pred = self.likelihood.predictive_new(m_F_pred, v_F_pred, output_function_ind)
