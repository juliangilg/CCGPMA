...
class HetLikelihood(Likelihood):
    ...
    def generate_metadata(self, iAnn=None):
        """
        ...
        #Aded by Julian, to incorporate multiples annotators
        if isinstance(iAnn, list):
            assert len(iAnn) == len(self.likelihoods_list)
        else:
            assert len(self.likelihoods_list)
        ...
        for t, lik in enumerate(self.likelihoods_list):
            dim_y, dim_f, dim_pred = lik.get_metadata(iAnn[t])
        ...
        metadata = {'task_index': t_index, 'y_index': np.int_(y_index[0,1:]), 'function_index': np.int_(f_index[0,1:]),
                    'd_index': np.int_(d_index[0,1:]),'pred_index': np.int_(p_index[0,1:]), 'iAnn': iAnn}
        return metadata
    ...
    def var_exp(self, Y, mu_F, v_F, Y_metadata):
        ...
        iAnn  = Y_metadata['iAnn']
        tasks = np.unique(t_ind)
        var_exp = []
        for t in tasks:

            ve_task = self.likelihoods_list[t].var_exp(Y[t], mu_F[t], v_F[t], Y_metadata=iAnn[t])
    ...
    def var_exp_derivatives(self, Y, mu_F, v_F, Y_metadata,Gauss_Newton):
        ...
        var_exp_dv = []
        for t in tasks:
            ve_task_dm, ve_task_dv = self.likelihoods_list[t].var_exp_derivatives(Y[t], mu_F[t], v_F[t], GN=Gauss_Newton,Y_metadata=iAnn[t])
    ...
    def predictive_new(self, mu_F_pred, v_F_pred, Y_metadata):
        """
        Returns a list of predictive mean and variance from all likelihoods.
        """
        t = Y_metadata
        m_pred, v_pred = self.likelihoods_list[t].predictive(mu_F_pred[0], v_F_pred[0], Y_metadata=None)
